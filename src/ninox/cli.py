"""Ninox CLI — maritime AIS/RF validation."""
...
def cmd_dashboard(args) -> int:
    from ninox.dashboard.export import export_dashboard
    out = export_dashboard(args.processed, args.out)
    print(f"✓ Dashboard: {out}")
    return 0

def cmd_coverage(args) -> int:
    from ninox.rf.coverage import cluster_summary
    print(json.dumps(cluster_summary(), indent=2))
    return 0

def cmd_rf_demo(args) -> int:
    from ninox.rf.simulate import run_rf_simulation
...
def cmd_demo(args) -> int:
    ...
    dash = export_dashboard(out_dir, out_dir / "Ninox_Investor_Dashboard.html")
...
def build_parser():
    p = argparse.ArgumentParser(prog="ninox", description="Ninox maritime AIS/RF validation")
    ...
    s.add_argument("--out", default="artifacts/Ninox_Investor_Dashboard.html")
