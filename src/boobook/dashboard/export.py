...
<title>Boobook — Source Data Validation Dashboard</title>
<style>
...
<body>
<header>
<h1>BOOBOOK — Source Data Validation Dashboard</h1>
...
Columns expected: <code>mmsi,timestamp,lat,lon,sog,cog,vessel_name,vessel_type</code>. For raw AMSA/CTS files, run <code>boobook validate-ais</code> first.
...
<footer>Boobook MVP v0.3 — AIS validation now, passive RF/TDOA validation next.</footer>
<script>
const DATA = __DATA__;
