# Boobook Investor MVP — Run Report

Generated: 2026-06-12

## What was fixed

- Integrated the AIS source-data validation dashboard into the main Boobook package.
- Replaced loose scripts with a proper `src/boobook` Python package.
...
boobook --help
boobook validate-ais data/sample/sample_ais_events.csv --out /tmp/boobook_processed
boobook dashboard --processed /tmp/boobook_processed --out /tmp/boobook_dashboard.html
boobook rf-demo --out /tmp/boobook_rf
python -m pytest
```
...
> Boobook validates the maritime analytics workflow on AIS source data today. The next proof point is live AIS capture via SDR, then controlled three-node TDOA, then RF/AIS mismatch detection inside an instrumented choke point.
